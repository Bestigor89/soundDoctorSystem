    ...
    <div class="md:flex-col md:items-stretch md:min-h-full md:flex-nowrap px-0 flex flex-wrap items-center justify-between w-full mx-auto">
        ...
        <div class="md:flex md:flex-col md:items-stretch md:opacity-100 md:relative md:mt-4 md:shadow-none shadow absolute top-0 left-0 right-0 z-40 overflow-y-auto overflow-x-hidden h-auto items-center flex-1 rounded hidden" id="example-collapse-sidebar">
            ...
            <ul class="md:flex-col md:min-w-full flex flex-col list-none">
                ...
                @can('mod_access')
                    <li class="items-center">
                        <a class="{{ request()->is("admin/mods*") ? "sidebar-nav-active" : "sidebar-nav" }}" href="{{ route("admin.mods.index") }}">
                            <i class="fa-fw c-sidebar-nav-icon fas fa-cogs">
                            </i>
                ...
                @endcan
                @can('file_library_access')
                    <li class="items-center">
                        <a class="{{ request()->is("admin/file-libraries*") ? "sidebar-nav-active" : "sidebar-nav" }}" href="{{ route("admin.file-libraries.index") }}">
                            <i class="fa-fw c-sidebar-nav-icon fas fa-cogs">
                            </i>
                            {{ trans('cruds.fileLibrary.title') }}
                        </a>
                    </li>
                @endcan
                @can('cost_access')
                ...
                @endcan
                @can('task_for_patient_access')
                    <li class="items-center">
                        <a class="{{ request()->is("admin/task-for-patients*") ? "sidebar-nav-active" : "sidebar-nav" }}" href="{{ route("admin.task-for-patients.index") }}">
                            <i class="fa-fw c-sidebar-nav-icon fas fa-cogs">
                            </i>
                            {{ trans('cruds.taskForPatient.title') }}
                        </a>
                    </li>
                @endcan
                @can('file_fore_mod_access')
                    <li class="items-center">
                        <a class="{{ request()->is("admin/file-fore-mods*") ? "sidebar-nav-active" : "sidebar-nav" }}" href="{{ route("admin.file-fore-mods.index") }}">
                            <i class="fa-fw c-sidebar-nav-icon fas fa-cogs">
                            </i>
                            {{ trans('cruds.fileForeMod.title') }}
                        </a>
                    </li>
                @endcan
