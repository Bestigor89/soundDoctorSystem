...
var table10, table12;
var hideOperation = false;

var condition =  "{}";
//if (!hideOperation) { condition = { title: "Операции" , "visible": false } }
condition = { title: "Операции" };

var col = new Array(
 [{ "title": "#" }, { "title": "Пользователь", "visible": false }, { "title": "Дата" },  { "title": "Состояние" }], // Задания
 [], // Сеансы-search
 [{ "visible": false }, { title: "Сорт", sWidth:'5%' }, condition, { title: "Название", sWidth: '90%' }, { title: "Длит <span id='tot'></span>" } ], // Сеансы
);
...
function generate_mus(param) {
    ...
    var table = $('#datatable_1'+param).DataTable( {
        serverSide: true,
        processing: true,
        ajax: 'ajax/data.php?item=1'+param+'&user_id='+$("#user_id").val()+'&client_id='+$("#client_id").val(),
        deferRender: true,
        language: {
        ...
        rowCallback: function( row, data ) {
          if(param == 0) {
              ...
              num = parseInt(data[3]);
              if ( num == -1 ) {
                $('td:eq(2)', row).html( '<b>ВЫПОЛНЕНО</b>' );
                _class = "YES";
              } else {
                if ( num == 0 ) {
                  if (formated_date > result )
                     $('td:eq(2)', row).html( '<b>НЕ ВЫПОЛНЕНО</b>' );
                  else
                     $('td:eq(2)', row).html( '<b>ВПЕРЕДИ</b>' );
                  _class = "NO";
			    } else {
                  _class = "HALF";
				}
...
$(document).ready(function() {
 ...
 // if (doctor == 1) $("#edt1").hide();
  ...
  $('#datatable_10 tbody').on('click', 'tr', function () {
		...
		$('#d1').val( arr[0] );
		$('#d2').val( arr[1] );
		console.log(data10[0] + " : " + data10[1] + " : " + data10[3]);
        ...
        if ( doctor == 2 ) {
            if (data10[3] == 0) {
                ...
                $("td.button button#bm1").attr('disabled', 'disabled');
                $("td.button button#bm2").attr('disabled', 'disabled');
                $("td.button button#bm3").attr('disabled', 'disabled');
                hideOperation = false;
            };
        }
        else  {
        ...
        console.log("hideOperation: " + hideOperation);
...
//console.log("send_job: "+q);
    ...
    break;
    case 2: //del
       $("td.button button#bm2").removeAttr('disabled');
...
<div id="ed_scr" class="scrolling">
  <table style="width:60%;">
    ...
    <td id="edt1">
      ...
      <div class="fixed">
        <table><tr>
        <td class="button" id="ed0">
          <button id="bm0" onclick="javascript:panel(0);" type="button" class="btn btn-default sd_btn_left">Новое задание</button><br />
          <button id="bm1" onclick="javascript:panel(1);" type="button" class="btn btn-default sd_btn_left" disabled>Редактировать</button><br />
          <button id="bm2" onclick="javascript:panel(2);" type="button" class="btn btn-default sd_btn_left" disabled>Удалить</button><br />
          <button id="bm3" onclick="javascript:panel(3);" type="button" class="btn btn-default sd_btn_left" disabled>Список</button><br />

          <div id="emenu" style="padding: 10px 3em; marging: 3px" >
            <button id="bn0" type="button" class="btn btn-default sd_btn_left">Добавить</button><br />
          </div>
        </td><td id="ed1">
          <span>Дата / Время</span><br />
          <input id="d1" class="form-control" style="width:65%;" placeholder="гггг-мм-дд"/>
          <input id="d2" class="form-control" style="width:65%;" placeholder="чч:мм:сс"/>
          <button id="send" onclick="javascript:send_job(1);" value="enter" type="button" class="btn btn-default sd_btn_right">Сохранить</button>
        </td>
        </tr></table>
    </div>
    </td>
